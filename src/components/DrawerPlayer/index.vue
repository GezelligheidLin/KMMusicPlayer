<script setup lang="ts">
  ...
  import { CITY_CODE, PlayMode } from '@/enum'
  import { Comment } from '@/api/interface'
  import { commentMusic } from '@/api'
  import { useDebounceFn } from '@vueuse/core'
  import { getCity, getWeatherApi } from '@/api/map.ts'
  ...
  function handleClickLyrics(item) {
    console.log(item, 'item')
    const secondTime = parseFloat(item.time / 1000)
    // 将时间转成秒数
    changeCurrentTime(secondTime)
  }
  ...
  async function getWeather() {
    const cityRes = await getCity({
      key: import.meta.env.VITE_APP_GAO_DE_KEY,
    })
    weather.value = await getWeatherApi({
      key: import.meta.env.VITE_APP_GAO_DE_KEY,
      city: CITY_CODE[cityRes.city],
    })
  }
            ...
            <div class="flex flex-col items-center w-96 mt-5">
              <div class="flex gap-1 items-center justify-center">
                <el-button
                  text
                  circle
                  @click="setPlayMode(PlayMode.Sequence)"
                  :class="{
                    '!bg-[#E4DADAFF] dark:!bg-[#453B3BFF]':
                      musicPlayMode === PlayMode.Sequence,
                  }"
                >
                ...
                <el-button
                  text
                  circle
                  @click="setPlayMode(PlayMode.Random)"
                  :class="{
                    '!bg-[#E4DADAFF] dark:!bg-[#453B3BFF]':
                      musicPlayMode === PlayMode.Random,
                  }"
                >
                ...
                <el-button
                  text
                  circle
                  @click="setPlayMode(PlayMode.Single)"
                  :class="{
                    '!bg-[#E4DADAFF] dark:!bg-[#453B3BFF]':
                      musicPlayMode === PlayMode.Single,
                  }"
                >
                  <icon-cil:loop-1 />
                </el-button>
